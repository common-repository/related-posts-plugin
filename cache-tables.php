...
$rpp_storage_class = 'RPP_Cache_Tables';

define('RPP_TABLES_RELATED_TABLE', 'rpp_related_cache');
define('RPP_TABLES_KEYWORDS_TABLE', 'rpp_keyword_cache');

class RPP_Cache_Tables {
	...
	function is_enabled() {
		...
		if (array_search($wpdb->prefix . RPP_TABLES_RELATED_TABLE,$tabledata) !== false &&
			array_search($wpdb->prefix . RPP_TABLES_KEYWORDS_TABLE,$tabledata) !== false)
			return true;
		else
			return false;
	}
	...
	function setup() {
		global $wpdb;
		$wpdb->query("CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}" . RPP_TABLES_KEYWORDS_TABLE . "` (
			`ID` bigint(20) unsigned NOT NULL default '0',
			`body` text NOT NULL,
			`title` text NOT NULL,
			`date` timestamp NOT NULL default CURRENT_TIMESTAMP,
			PRIMARY KEY  (`ID`)
			) ENGINE=MyISAM COMMENT='RPP''s keyword cache table';");
		$wpdb->query("CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}" . RPP_TABLES_RELATED_TABLE . "` (
	...
	function upgrade($last_version) {
		...
		if (version_compare('3.2.1b4', $last_version) > 0) {
			...
			// are no duplicates.
			$this->flush();
			$wpdb->query('ALTER TABLE ' . $wpdb->prefix . RPP_TABLES_RELATED_TABLE .
			  ' DROP PRIMARY KEY ,' .
	...
	function cache_status() {
		...
		return $wpdb->get_var("select (count(p.ID)-sum(c.ID IS NULL))/count(p.ID)
			...
			LEFT JOIN `{$wpdb->prefix}" . RPP_TABLES_RELATED_TABLE . "` as c ON (p.ID = c.reference_ID)
	...
	function join_filter($arg) {
		global $wpdb;
		if ($this->rpp_time)
			$arg .= " join {$wpdb->prefix}" . RPP_TABLES_RELATED_TABLE . " as rpp on {$wpdb->posts}.ID = rpp.ID";
		return $arg;
	}
	...
	function where_filter($arg) {
		...
		if ($this->rpp_time) {
			...
			$arg = str_replace("$wpdb->posts.ID = ","rpp.score >= $threshold and rpp.reference_ID = ",$arg);
	...
	function orderby_filter($arg) {
		global $wpdb;
		if ($this->rpp_time and $this->score_override)
			$arg = str_replace("$wpdb->posts.post_date","rpp.score",$arg);
		return $arg;
	}
	...
	function is_cached($reference_ID) {
		global $wpdb;
		return $wpdb->get_var("select count(*) as count from {$wpdb->prefix}" . RPP_TABLES_RELATED_TABLE . " where reference_ID = $reference_ID");
	}

	function clear($reference_ID) {
		global $wpdb;
		if (is_array($reference_ID) && count($reference_ID))
			$wpdb->query("delete from {$wpdb->prefix}" . RPP_TABLES_RELATED_TABLE . " where reference_ID in (".implode(',',$reference_ID).")");
		else if (is_int($reference_ID))
			$wpdb->query("delete from {$wpdb->prefix}" . RPP_TABLES_RELATED_TABLE . " where reference_ID = {$reference_ID}");
	}
	...
	function update($reference_ID) {
		...
		if ( !is_int( $reference_ID ) )
			return new WP_Error('rpp_cache_error', "update's reference ID must be an int" );
		...
		$wpdb->query("insert into {$wpdb->prefix}" . RPP_TABLES_RELATED_TABLE . " (reference_ID,ID,score) ".rpp_sql(array(),true,$reference_ID)." on duplicate key update date = now()");
		...
		if ($wpdb->rows_affected) {
			...
			$wpdb->query("insert into {$wpdb->prefix}" . RPP_TABLES_RELATED_TABLE . " (reference_ID,ID,score) values ($reference_ID,0,0) on duplicate key update date = now()");
			if (!$wpdb->rows_affected)
				return false;
	...
	function flush() {
		global $wpdb;
		return $wpdb->query("truncate table `{$wpdb->prefix}" . RPP_TABLES_RELATED_TABLE . "`");
	}
	...
	function related($reference_ID = null, $related_ID = null) {
		...
		if (!is_null($reference_ID) && !is_null($related_ID)) {
			$results = $wpdb->get_col("select ID from {$wpdb->prefix}" . RPP_TABLES_RELATED_TABLE . " where reference_ID = $reference_ID and ID = $related_ID");
			return count($results) > 0;
		}

		// return a list of ID's of "related" entries
		if (!is_null($reference_ID)) {
			return $wpdb->get_col("select distinct ID from {$wpdb->prefix}" . RPP_TABLES_RELATED_TABLE . " where reference_ID = $reference_ID and ID != 0");
		}

		// return a list of entities which list this post as "related"
		if (!is_null($related_ID)) {
			return $wpdb->get_col("select distinct reference_ID from {$wpdb->prefix}" . RPP_TABLES_RELATED_TABLE . " where ID = $related_ID");
		}
	...
	function cache_keywords($ID) {
		...
		$wpdb->query("insert into {$wpdb->prefix}" . RPP_TABLES_KEYWORDS_TABLE . " (ID,body,title) values ($ID,'$body_terms ','$title_terms ') on duplicate key update date = now(), body = '$body_terms ', title = '$title_terms '");

	}

	function get_keywords($ID, $type='body') {
		global $wpdb;
		$out = $wpdb->get_var("select $type from {$wpdb->prefix}" . RPP_TABLES_KEYWORDS_TABLE . " where ID = $ID");
		if ($out === false or $out == '') { // if empty, try caching them first.
			$this->cache_keywords($ID);
			$out = $wpdb->get_var("select $type from {$wpdb->prefix}" . RPP_TABLES_KEYWORDS_TABLE . " where ID = $ID");
		}
		if ($out === false or $out == '') { // if still empty... return false
			//echo "<!--RPP ERROR: couldn't select/create rpp $type keywords for $ID-->";
			return false;
		} else {
			return $out;
		}
