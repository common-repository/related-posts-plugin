...
if ($related_query->have_posts()) {
	while ($related_query->have_posts()) {
		...
		if (current_user_can('manage_options') && $domain != 'rss')
			$output .= ' <abbr title="'.sprintf(__('%f is the RPP match score between the current entry and this related entry. You are seeing this value because you are logged in to WordPress as an administrator. It is not shown to regular visitors.','rpp'),round(get_the_score(),3)).'">('.round(get_the_score(),3).')</abbr>';
		$output .= '</a>';
		if ($show_excerpt) {
			$output .= $before_post .
			  rpp_excerpt(get_the_excerpt(),$excerpt_length)
			  . $after_post;
		}
		$output .=  $after_title."\n";
